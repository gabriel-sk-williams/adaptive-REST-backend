...
const getNodeByHex = function (session, hex) {
  return session.readTransaction(tx => (
    tx.run(q.qbHex, {hex:hex}))).then(r => { return r });
};

const queryByLemma = function (session, lemma) {
  return session.readTransaction(tx => (
    tx.run(q.qbLemma, {lemma:lemma}))).then(r => { return r });
};

const queryByAddress = function (session, address) {
  return session.readTransaction(tx => (
    tx.run(q.qbAddress, {address:address}))).then(r => { return r });
};

const queryByName = function (session, name) {
  return session.readTransaction(tx => (
    tx.run(q.qbName, {name:name}))).then(r => { return r });
};

module.exports = { // export exposed functions
  getNodeByHex,
  queryByLemma,
  queryByAddress,
  queryByName
